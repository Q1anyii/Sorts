...
- **织历视图**：按日 / 周聚合日程与专注记录，支持**月份下拉快速切换**（近 5 年月度选项）与今日概览，直接反映「哪一天织得密」；**任务点分色**——用户自定义颜色标记优先，否则按紧急度（URGENT 红 / HIGH 琥珀 / MEDIUM 蓝 / LOW 灰）；**点击任意日期跳转织程并携带 date 参数**，自动按该日期闭区间筛选；日历读独立**全量日程列表**，不受织程日期筛选影响（修复筛选后圆点丢失需刷新）
