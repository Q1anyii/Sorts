...
32. **`UpdateSchedule` 工具缺独立单测**（2026-09-20 数据核查发现）：`sorts-ai` 注册的 7 个 AI 工具中只有 6 个有对应 `*ToolTest`（`CreateSchedule` / `CreateSchedules` / `GetProfile` / `QueryPoints` / `QuerySchedules` / `QueryStatistics`），**`UpdateScheduleTool` 在 `src/test` 下零命中**。它的行为仅被 `ToolRegistryTest` 的注册与双钥匙机制间接覆盖，工具自身的参数组装 / `ToolJsonCodec` 序列化没有直测。**对外表述「7 工具（4 读 3 写）」时，不得暗示 7 个工具都已有单测。**

33. **测试规模的三套口径必须分清**（2026-09-20 数据核查固化）：`*IT` 不在 surefire 默认 includes 内，只在 `-Pintegration` profile 下被收录。故 —— **业务单测口径** `cd backend && ./mvnw test` = 42 类 / 392 例（41 业务单测类 367 例 + 指标测试类 25 例）；**含集成的全量口径** 追加 `UserServiceIT` 1 类 4 例 = **43 类 / 396 例**。**CI 的 `backend-test` 分两步把两者都跑了**（`clean verify` + `-Pintegration test -Dtest='*IT'`），所以「43 类 / 396 例全绿」是 CI 口径、成立；但被问「396 怎么跑的」时，答案必须是上面这两条命令，不能说成单条 `mvn test`。**「25 例安全专项」的唯一出处是 `sorts-metrics-tests`**（5 组 `@Nested`：双钥匙 6 + 状态机 4 + 多日规划 5 + 兑换并发 4 + 网关收口 6），不是另有一套安全测试。
